...
public class ParentServiceTests {
    ...
    @Test
    public void fetchParentEntity_successful(){
        ...
        ParentEntity result = service.fetchParentEntity(parentId);
    ...
    @Test
    public void fetchParentEntity_entityNotFound(){
        Integer parentId = 123;

        when(repository.findById(parentId)).thenReturn(Optional.empty());

        assertThatThrownBy(() -> service.fetchParentEntity(parentId))
